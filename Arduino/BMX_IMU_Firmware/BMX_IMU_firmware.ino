...
bool enableYawReading = true; //transmit yaw first and the the others
bool enablePitchReading  = false; 
bool enableRollReading  = false; 
...
//    imuLog += String(imu.yaw, 2) + ", ";
    ...
    if (enableYawReading) // If Euler-angle logging is enabled
  {
    imuLog +=  "yaw=" + String(int(imu.yaw*1000));
    enableYawReading = false;
    enablePitchReading = true;
  }

  else if (enablePitchReading) // If Euler-angle logging is enabled
  {
    imuLog +=  "pitch=" + String(int(imu.pitch*1000));
    enablePitchReading = false;
    enableRollReading = true;
  }

  else if (enableRollReading) // 
  {
    imuLog +=  "roll=" + String(int(imu.roll*1000));
    enableRollReading = false;
    enableYawReading = true;
  }
